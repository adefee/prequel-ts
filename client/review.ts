...
if (Number.isFinite(savedTreeWidth)) setTreeWidth(savedTreeWidth);
...
document.addEventListener('click', (e) => {
  ...
  if (segBtn) {
    ...
    if (param && value) goToParam(param, value);
    return;
  }
...
function initRepoPicker(): void {
  ...
  if (!pickerEl || !toggleEl || !menuEl) return;
  ...
  const renderMenu = (): void => {
    ...
    if (!shortcuts.length) {
      ...
      for (const repoPath of shortcuts) menuEl.appendChild(shortcutRow(repoPath, current));
    }
  ...
  document.addEventListener('click', (e) => {
    if (!menuEl.hidden && e.target instanceof Node && !pickerEl.contains(e.target)) closeMenu();
  });
