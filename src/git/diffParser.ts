...
import type { FileStatus, ReviewDiff, ReviewFile, ReviewHunk, ReviewLine } from '../types';
...
function status(file: StructuredPatch): FileStatus {
  ...
  if (file.isCreate) return 'added';
  if (file.isDelete) return 'removed';
  return 'modified';
}
...
function mapFile(file: StructuredPatch): ReviewFile {
  ...
  return {
    id: fileId(oldPath, newPath), oldPath, newPath, status: reviewStatus,
...
export function parseDiff(patch: string): ReviewDiff {
  return { files: parsePatch(patch).map(mapFile) };
}
