...
afterAll(async () => Promise.all(tmpDirs.map((dir) => fs.rm(dir, { recursive: true, force: true }))));
...
describe('install', () => {
  test('installs, recognizes, protects, and force-replaces one real file', async () => {
    ...
    expect(installed).toMatchObject({
      status: 'installed', dest: path.join(cwd, '.claude', 'skills', 'prequel', 'SKILL.md'),
    });
  ...
  test('refuses symbolic-link destinations and ancestors without clobbering', async () => {
    ...
    await expect(install('claude', { project: true, force: true, cwd })).rejects.toThrow(/symbolic link/);
