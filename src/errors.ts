...
export function codeOf(err: unknown): ErrorCode | null {
  ...
  if (err.status === 403) return 'cross-origin';
  if (err.status === 413) return 'body-too-large';
  if (/no repo/i.test(err.message)) return 'no-repo';
  if (err.status === 400) return 'bad-params';
  return null;
}
